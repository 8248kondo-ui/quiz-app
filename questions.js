...
const questionDatabase = [
    ...
    },
    {
        ...
        "answer": 0,
        "explanation": "正解は1番です。\n\nHTMLドキュメント内でJavaScriptを記述する際は、<script>タグを使用します。また、外部ファイル（拡張子 .js）に記述したJavaScriptコードを <script src=\"path/to/file.js\"></script> のように読み込んで実行することも一般的です。\n\n他の選択肢について：\n・<javascript> タグや .java 拡張子は誤りです（Java用の拡張子です）。\n・<style> や .css はCSSスタイルシート用です。\n・<body> タグの属性だけでなく、外部ファイルや <script> タグの内部にも記述できます。",
        "category": "JavaScript"
    },
    {
        "id": 472,
        "question": "【変数宣言의constとletの違い】\n問2. 以下のコードはどのような処理を行うか。正しいものを選べ。\n\nconst user = { name: \"Taro\" };\nuser.name = \"Hanako\";\nuser = { name: \"Jiro\" };",
        "options": [
    ...
    },
    {
        ...
        "answer": 1,
        "explanation": "正解は2番です。\n\nアロー関数において引数が2つ以上ある場合、引数を囲むカッコ「(a, b)」を省略することはできません（引数が1つのときのみカッコを省略できます）。よって「const add = a, b => a + b;」は構文エラーとなります。\n\n他の選択肢はすべて正しいアロー関数の構文です。3番のように波カッコ { } と return を同時に省略して、式を簡潔に書く（暗黙のreturn）ことも可能です。",
        "category": "JavaScript"
    },
    {
        "id": 475,
        ...
        "answer": 1,
        "explanation": "正解は2番です。\n\nJavaScriptは「動的型付け言語」であり、変数そのものには特定の型が固定されません。そのため、同じ変数であっても、最初は数値を代入し、後から文字列やオブジェクトなど異なるデータ型の値を代入し直すことが可能です（これが「値が異なる変数に異なるデータ型の値を代入することができる」という意味に相当します）。\n\n1番は静的型付け言語（Javaなど）の説明です。\n3番はオブジェクトや配列なども変数に代入できるため誤りです。\n4番は暗黙的な型変換（自動型変換）が行われるため、常に明示的なキャストが必要というわけではなく、誤りです。",
        "category": "JavaScript"
    },
    {
        "id": 476,
        ...
        "answer": 1,
        "explanation": "正解は2番です。\n\nJavaScriptにおいて、波カッコ「{}」はオブジェクトリテラルを表し、空のオブジェクト（プロパティを持たないオブジェクト）を生成して変数 obj に代入します。\n\n1番の「空の配列オブジェクト」を生成する場合は、角カッコ「[]」を使用します（例: const arr = [];）。",
        "category": "JavaScript"
    },
    {
        "id": 477,
        ...
        "answer": 1,
        "explanation": "正解は2番です。\n\nArray.prototype.map() メソッドは、元の配列の各要素に対してコールバック関数を実行し、その実行結果を格納した「新しい配列」を作成して返します。元の配列である numbers は変更されません（イミュータブルな操作）。\n\nしたがって、result には [2, 4, 6] が代入され、numbers は [1, 2, 3] のまま維持されます。",
        "category": "JavaScript"
    },
    {
        "id": 478,
        ...
        "answer": 0,
        "explanation": "正解は1番です。\n\nコンストラクタ関数を用いてオブジェクトを生成する際は、関数内部で「this.プロパティ名 = 値」の形式で新しく作成されるインスタンスのプロパティを設定します。\n\n他の選択肢について：\n・2番はローカル変数を宣言しているだけで、生成されるオブジェクトのプロパティにはなりません。\n・3番はクラス宣言の構文として正しくありません（class User { constructor(name, age) { ... } } のように書く必要があります）。\n・4番は User というコンストラクタ関数自体のプロパティ（静的プロパティ）を設定しようとしているため、生成されるインスタンス of プロパティにはなりません。",
        "category": "JavaScript"
    },
    {
        "id": 479,
        ...
        "answer": 0,
        "explanation": "正解は1番です。\n\nDOM（Document Object Model）とは、HTMLやXMLドキュメントをツリー構造の「オブジェクト」として表現し、JavaScriptなどのプログラミング言語から要素の取得・変更・削除などの操作を行えるようにするための標準的なAPIです。",
        "category": "JavaScript"
    },
    {
        "id": 480,
    ...
    },
    {
        ...
        "answer": 0,
        "explanation": "正解は1番です。\n\nあるノードの直後にある兄弟ノードを参照するには「nextSibling」プロパティを使用します。また、Element（要素ノード）に限定して次の兄弟要素を取得したい場合は「nextElementSibling」を使用します。\n\n他の選択肢について：\n・previousSibling は直前にある兄弟ノードを参照します。\n・childNodes (複数形) は子ノードのリストを参照します。\n・parentNode は親ノードを参照します。",
        "category": "JavaScript"
    },
    {
        "id": 482,
    ...
    },
    {
        ...
        "answer": 0,
        "explanation": "正解は1番です。\n\n要素をクリックした時の処理を登録するには、要素の「onclick」プロパティに関数を代入するか、または「addEventListener(\"click\", コールバック関数)」を使用します。注意点として、addEventListenerを使用する際のイベント名は「onclick」ではなく「click」（onは付けない）になります。したがって、3番は誤りです。\n\n2番、4番の hover や press は正しいクリックイベント用のものではありません（ホバーはmouseenter/mouseleave等、マウスボタン押し下げはmousedown等を使用します）。",
        "category": "JavaScript"
    }
];
